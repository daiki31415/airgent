...
describe("ConfigManager MCP", () => {
  ...
  test("loadMCPServers reads saved servers", () => {
    const cm = new ConfigManager();
    const testServers = [
      { name: "playwright", type: "local" as const, command: ["npx", "playwright"], enabled: true },
    ];
  ...
  test("saveMCPServers persists to disk", () => {
    const cm = new ConfigManager();
    const servers = [
      { name: "s1", type: "local" as const, command: ["echo", "hi"], enabled: true },
      { name: "s2", type: "remote" as const, url: "https://example.com/mcp", enabled: false },
    ];
