...
describe("DeviceSync", () => {
  ...
  test("git command failure throws error", () => {
    (Bun as any).spawnSync = createSpawnMock({ exitCode: 1, stderr: Buffer.from("fatal: not a git repository") });
  ...
  test("git command result parsing returns stdout and stderr", () => {
    (Bun as any).spawnSync = createSpawnMock({ stdout: Buffer.from("on branch main") });
