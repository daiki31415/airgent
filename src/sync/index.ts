...
export class DeviceSync {
	...
	private exportData(): void {
		...
		writeFileSync(this.exportFile, JSON.stringify(data, null, 2), {
			mode: 0o600,
		});
		this.logger.info(
			`Exported ${compressed.length} compressed, ${Object.keys(metadata).length} metadata, ${sessions.length} sessions`,
		);
	}
	...
	private importData(): void {
		...
		this.logger.info(
			`Imported ${data.compressed?.length || 0} compressed, ${Object.keys(data.metadata || {}).length} metadata keys`,
		);
	}
