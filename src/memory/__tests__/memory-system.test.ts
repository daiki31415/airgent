...
describe("MemorySystem", () => {
  ...
  test("createMemory stores all fields correctly", () => {
    ...
    const id = ms.createMemory({
      sessionId: "s1", bug: "test bug", investigation: "investigated",
      rootCause: "root cause", fix: "the fix", reason: "because",
      evidence: [
        { type: "observed", content: "evidence 1", source: "log", timestamp: 100 },
        { type: "verified", content: "evidence 2", source: "test", timestamp: 200 },
      ],
      confidence: 0.75, tags: ["tag1", "tag2"],
      files: ["src/file1.ts"], commands: ["npm test"],
    });
  ...
  test("createMemory stores multiple evidence entries", () => {
    ...
    const id = ms.createMemory({
      sessionId: "s1", bug: "bug", investigation: "", rootCause: "", fix: "", reason: "",
      evidence: [
      ...
      confidence: 0.5, tags: [], files: [], commands: [],
    });
  ...
  test("findRelevant sorts by confidence descending", () => {
    ...
    storage.insertMemory({
      id: "m1", sessionId: "s1", bug: "low conf", investigation: "", rootCause: "", fix: "",
      reason: "", confidence: 0.4, tags: ["test"], files: [], commands: [],
    });
    storage.insertMemory({
      id: "m2", sessionId: "s1", bug: "high conf", investigation: "", rootCause: "", fix: "",
      reason: "", confidence: 0.9, tags: ["test"], files: [], commands: [],
    });
  ...
  test("createMemory with empty evidence stores no evidence", () => {
    ...
    const id = ms.createMemory({
      sessionId: "s1", bug: "bug", investigation: "", rootCause: "", fix: "", reason: "",
      evidence: [], confidence: 0.5, tags: [], files: [], commands: [],
    });
  ...
  test("createMemory auto-links to existing memories with matching tags", () => {
    ...
    storage.insertMemory({
      id: "existing", sessionId: "s1", bug: "old bug", investigation: "", rootCause: "", fix: "",
      reason: "", confidence: 0.9, tags: ["crash", "memory"], files: [], commands: [],
    });

    // Create new memory with overlapping tag
    const newId = ms.createMemory({
      sessionId: "s1", bug: "new crash", investigation: "", rootCause: "", fix: "", reason: "",
      evidence: [{ type: "observed", content: "crash", source: "log", timestamp: 0 }],
      confidence: 0.8, tags: ["crash", "null"], files: [], commands: [],
    });
  ...
  test("getEvidence returns empty array for memory with no evidence", () => {
    ...
    const storage = (ms as any).storage as Storage;
    storage.insertMemory({
      id: "m1", sessionId: "s1", bug: "b1", investigation: "", rootCause: "", fix: "",
      reason: "", confidence: 0.5, tags: [], files: [], commands: [],
    });
    const evidence = ms.getEvidence("m1");
    expect(evidence).toHaveLength(0);
  ...
  test("createMemory handles very large bug text", () => {
    ...
    const id = ms.createMemory({
      sessionId: "s1", bug: longBug, investigation: "", rootCause: "", fix: "", reason: "",
