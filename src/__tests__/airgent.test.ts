...
mock("readline", () => ({ createInterface: mock(() => mockReadlineInterface) }));
mock.module(resolve(import.meta.dir, "../../node_modules/@opentui/core"), () => {
  const r = () => mock(() => {
    const obj: any = { content: "", fg: "", width: "", add: mock(), remove: mock(), findDescendantById: mock(() => null), flexDirection: "" };
    return obj;
  });
  return {
    ...
    Text: r(),
    ScrollBox: r(),
    Input: mock(() => ({ value: "", on: mock(), focus: mock(), focusable: false })),
    Box: r(),
    Select: mock(() => ({ on: mock(), focus: mock(), focusable: false, getSelectedOption: mock(() => ({ value: null })) })),
  };
});
...
const mockSettings = {
  maxSystemPromptTokens: 3000, maxContextTokens: 32000,
  uiRefreshIntervalMs: 100, autoCompressThreshold: 0.7,
  watchdogIntervalMs: 5000, maxRetriesPerNode: 3,
  memoryAutoLink: true, showPipelineProgress: false, debug: false,
};
const mockModelEntry = { provider: "test", model: "gpt-4" };

const mockConfig = {
  constitution: {
    name: "Test Constitution", version: "1.0.0",
    principles: ["Be robust"], constraints: ["Never delete"],
    ethical_guidelines: ["Be helpful"],
  },
  persona: {
    name: "Test Bot", role: "tester", tone: "professional", rules: ["Explain"],
  },
  models: {
  ...
  return {
    ...
    api: {
      ...
      listProviders: mock(() => ({
        ...
        all: [{ id: "test-provider", name: "Test Provider", models: { "gpt-4": {} } }],
      })),
      chat: mock(() => ({ content: "mock response" })),
      streamChat: mock(function* () { yield "chunk1"; yield "chunk2"; }),
      setAuth: mock(),
      listMCP: mock(() => ({})),
      addMCP: mock(),
      connectMCP: mock(),
    ...
    promptManager: {
      buildSystemPrompt: mock(() => ({ prompt: "System prompt", tokenCount: 50 })),
    ...
    validation: {
      ...
      switchModel: mock(),
      validate: mock(() => ({
        contradictions: 0, circularReferences: 0, hallucinatedLinks: 0,
        inferenceAsFact: 0, issues: [], overallHealth: "healthy" as const,
      })),
    },
    watchdog: {
      init: mock(),
    ...
    logger: {
      ...
      setDebug: mock(),
      child: mock(() => ({
        info: mock(), warn: mock(), error: mock(), debug: mock(), fatal: mock(), setDebug: mock(),
      })),
    },
  };
}

function clearAllMocks(mocks: ReturnType<typeof makeMockInstances>) {
  const all = [
...
describe("Airgent — Command Handling", () => {
  ...
  test("/copy warns when nothing to copy", async () => {
    ...
    expect(mocks.ui.log).toHaveBeenCalledWith("warn", "airgent", expect.stringContaining("Nothing to copy"));
  });
  ...
  test("/copy shows error on clipboard failure", async () => {
    mocks.ui.copy = mock(() => ({ success: false, method: "file", error: "failed" }));
    await sendInput("/copy text");
    expect(mocks.ui.log).toHaveBeenCalledWith("error", "airgent", expect.stringContaining("Copy failed"));
  });
  ...
  test("/providers lists providers", async () => {
    ...
    expect(mocks.ui.log).toHaveBeenCalledWith("info", "providers", expect.stringContaining("Connected"));
  });

  test("/providers handles API error", async () => {
    mocks.api.listProviders = mock(() => { throw new Error("fail"); });
    await sendInput("/providers");
  ...
  test("/mcp list shows configured servers", async () => {
    agent.configManager.loadMCPServers = mock(() => [
      { name: "my-srv", type: "local", command: ["node", "srv.js"], enabled: true },
    ]);
  ...
  test("/mcp add saves local server", async () => {
    const saveSpy = mock();
    agent.configManager = {
      loadMCPServers: mock(() => []), saveMCPServers: saveSpy,
      needsConfig: mock(() => false), load: mock(() => mockConfig),
      saveSettings: mock(), saveModels: mock(), getModels: mock(() => mockConfig.models),
    };
  ...
  test("/mcp add prevents duplicates", async () => {
    ...
    expect(mocks.ui.log).toHaveBeenCalledWith("warn", "mcp", expect.stringContaining("already exists"));
  });
  ...
  test("/mcp unknown subcommand warns", async () => {
    ...
    expect(mocks.ui.log).toHaveBeenCalledWith("warn", "mcp", expect.stringContaining("Unknown subcommand"));
  });
  ...
  test("rate limited input returns early", async () => {
    ...
    await sendInput("/help");
    expect(mocks.ui.log).toHaveBeenCalledWith("warn", "airgent", expect.stringContaining("Rate limit"));
  });
});
...
describe("Airgent — processTask flow", () => {
  ...
  test("processTask warns on high corruption", async () => {
    ...
    await agent.processTask("test");
    expect(mocks.ui.log).toHaveBeenCalledWith("warn", "inspector", expect.stringContaining("Corruption"));
  });
  ...
  test("processTask warns on unhealthy watchdog", async () => {
    mocks.watchdog.check = mock(() => ({
      healthy: false, actions: [{ type: "warning", message: "issue" }],
    }));
    await agent.processTask("test");
  ...
  test("processTask updates status", async () => {
    ...
    expect(mocks.ui.updateStatus).toHaveBeenCalledWith({ pipelineNode: "plan" });
    expect(mocks.ui.updateStatus).toHaveBeenCalledWith({ pipelineNode: "execute" });
    expect(mocks.ui.updateStatus).toHaveBeenCalledWith({ status: "completed", pipelineNode: "" });
  });

  test("processTask handles errors gracefully", async () => {
    mocks.planner.analyzeTask = mock(() => { throw new Error("plan failed"); });
    await agent.processTask("test");
    expect(mocks.ui.log).toHaveBeenCalledWith("error", "airgent", expect.stringContaining("plan failed"));
  });

  test("processTask sets error status on failure", async () => {
    mocks.planner.analyzeTask = mock(() => { throw new Error("fail"); });
    await agent.processTask("test");
...
describe("Airgent — buildAgentContext", () => {
  ...
  test("includes systemPrompt from PromptManager", () => {
    mocks.promptManager.buildSystemPrompt = mock(() => ({ prompt: "custom prompt", tokenCount: 50 }));
    const ctx: AgentContext = agent.buildAgentContext("task");
  ...
  test("tokenCount estimated from prompt and task length", () => {
    mocks.promptManager.buildSystemPrompt = mock(() => ({ prompt: "hello", tokenCount: 2 }));
...
describe("Airgent — Pipeline Handlers", () => {
  ...
  test("clarify handler uses streaming when showPipelineProgress", async () => {
    ...
    mocks.api.streamChat = mock(function* () { yield "streamed output"; });
    const h = handlers.get("clarify")!;
    const result = await h(new Map());
    expect(result).toHaveProperty("content");
  ...
  test("test handler passes clean output", async () => {
    ...
    mocks.api.chat = mock(() => ({ content: "all looks correct, reviewed and approved" }));
    const h = handlers.get("test")!;
    const result: any = await h(new Map());
  ...
  test("validate handler calls validation agent", async () => {
    mocks.validation.validate = mock(() => ({
      contradictions: 0, circularReferences: 0, hallucinatedLinks: 0,
      inferenceAsFact: 0, issues: [], overallHealth: "healthy",
    }));
  ...
  test("validate handler warns on unhealthy", async () => {
    mocks.validation.validate = mock(() => ({
      contradictions: 2, circularReferences: 1, hallucinatedLinks: 0,
      inferenceAsFact: 0, issues: ["i1"], overallHealth: "warning",
    }));
    const h = handlers.get("validate")!;
    await h(new Map());
    expect(mocks.ui.log).toHaveBeenCalledWith("warn", "validation", expect.stringContaining("Health"));
  });
...
describe("Airgent — Streaming (streamNodeOutput)", () => {
  ...
  test("streamNodeOutput stores in pipelineData field", async () => {
    mocks.api.streamChat = mock(function* () { yield "stored"; });
  ...
  test("streamNodeOutput shows node prefix", async () => {
    mocks.api.streamChat = mock(function* () { yield "data"; });
    await agent.streamNodeOutput(
      { provider: "test", model: "gpt-4" },
      [{ role: "user", content: "hi" }],
      "my-node", "plan",
...
describe("Airgent — updateStatus", () => {
  ...
  test("updateStatus with multiple fields", () => {
    agent.updateStatus({ sessionId: "s1", status: "running", pipelineNode: "plan" });
    expect(mocks.ui.updateStatus).toHaveBeenCalledWith({
      sessionId: "s1", status: "running", pipelineNode: "plan",
    });
  });
});
