...
export async function ensureOpenCodeServer(agent: ServerHandle): Promise<void> {
  ...
  if (health.healthy) {
    agent.ui.log("info", "airgent", "OpenCode server connected" + (health.version ? " v" + health.version : ""));
  } else {
    agent.ui.log("warn", "airgent", "OpenCode server not reachable. Set OPENCODE_SERVER_PASSWORD and run: opencode serve");
  }
