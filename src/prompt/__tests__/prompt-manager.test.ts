...
describe("PromptManager.buildSystemPrompt", () => {
	...
	test("includes runtime debug setting", () => {
		const pm = new PromptManager(
			minimalConfig({ settings: { ...minimalConfig().settings, debug: true } }),
			mockSkillsManager(),
		);
		const { prompt } = pm.buildSystemPrompt();
	...
	test("config debug=false produces correct output", () => {
		const pm = new PromptManager(
			minimalConfig({
				settings: { ...minimalConfig().settings, debug: false },
			}),
			mockSkillsManager(),
		);
		const { prompt } = pm.buildSystemPrompt();
...
describe("PromptManager.wouldExceedLimit", () => {
	...
	test("returns true when combined tokens exceed limit", () => {
		const pm = new PromptManager(
			minimalConfig({
				settings: { ...minimalConfig().settings, maxSystemPromptTokens: 1 },
			}),
			mockSkillsManager(),
		);
	...
	test("returns false when combined tokens are within limit", () => {
		const pm = new PromptManager(
			minimalConfig({
				settings: { ...minimalConfig().settings, maxSystemPromptTokens: 99999 },
			}),
			mockSkillsManager(),
		);
	...
	test("exact limit boundary (equal) returns true", () => {
		const pm = new PromptManager(
			minimalConfig({
				settings: { ...minimalConfig().settings, maxSystemPromptTokens: 0 },
			}),
			mockSkillsManager(),
		);
