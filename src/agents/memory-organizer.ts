...
export class MemoryOrganizerAgent extends BaseAgent {
  ...
  private analyzeLogs(logs: RawLogRow[]): LogPattern[] {
    ...
    for (const line of lines) {
      ...
      if (/bug:|error:|issue:/.test(lower)) {
        if (currentBug && currentFix) {
          patterns.push({
            bug: currentBug, investigation: currentInvestigation, rootCause: "",
            fix: currentFix, reason: currentReason,
            evidenceItems: [{ content: currentBug, source: "log" }, { content: currentFix, source: "log" }],
            files: [], commands: [],
          });
        }
        currentBug = line; currentInvestigation = ""; currentFix = ""; currentReason = "";
    ...
    return patterns.length > 0 ? patterns : [{
      bug: combinedLog.slice(0, 200), investigation: "", rootCause: "", fix: "", reason: "",
  ...
  private classifyEvidence(content: string, source: string): EvidenceType {
    ...
    if (src === "log" || src === "console" || lower.includes("output:") ||
        (!lower.includes("i think") && !lower.includes("probably") && !lower.includes("might"))) return "observed";
  ...
  private calculateConfidence(evidence: EvidenceEntry[]): number {
    ...
    const weights: Record<EvidenceType, number> = { verified: 1.0, observed: 0.8, inferred: 0.4, generated: 0.3 };
    const total = evidence.reduce((s, e) => s + (weights[e.type] || 0.3), 0);
    return Math.min(total / evidence.length + 0.1, 1.0);
  }

  private extractTags(pattern: { bug: string; fix: string; files: string[] }): string[] {
    const tags = new Set<string>();
    for (const file of pattern.files) {
      const ext = file.split(".").pop();
      if (ext) tags.add(ext);
    }
    for (const kw of ["error", "bug", "fix", "crash", "performance", "memory", "security", "type", "compile", "runtime", "async", "sync"]) {
