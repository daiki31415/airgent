...
import type { AgentContext, ModelEntry, OpenCodeResponse, WatchdogAction } from "../../types";
...
describe("WatchdogAgent.reset", () => {
  test("clears all internal state", () => {
    const agent = createWatchdog();
    agent.check({ failures: { worker: 3 }, retries: { planner: 5 }, currentTokens: 1000 });
