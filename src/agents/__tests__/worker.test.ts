...
import type { AgentContext, ModelEntry, OpenCodeResponse, CompressedEntry } from "../../types";
...
describe("WorkerAgent.execute (streaming with onChunk)", () => {
  ...
  test("onChunk is called with incremental content", async () => {
    const api = new (class extends OpenCodeAPI {
      chat = mock(async () => ({ id: "id", content: "", model: "m", usage: { promptTokens: 0, completionTokens: 0, totalTokens: 0 } }));
...
describe("WorkerAgent with context enhancement", () => {
  test("includes compressed context in prompt when available", async () => {
    const compression = new (class extends CompressionManager {
      constructor() { super(null as any, null as any); }
      override findForDecompression() {
        return [{
          id: "c1", originalId: "o1", title: "Auth fix",
          topics: ["auth", "login"], timestamp: 0,
          entities: [], files: [], commands: [], errorKeywords: [],
          importanceScore: 0.8, tokenCount: 100,
