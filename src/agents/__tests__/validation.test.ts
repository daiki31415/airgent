...
import { ValidationAgent, type ValidationReport } from "../validation";
import { OpenCodeAPI } from "../../api/opencode";
import { MemorySystem } from "../../memory";
import { Storage } from "../../storage";
import type { AgentContext, ModelEntry, OpenCodeResponse, StructuredMemory, MemoryLink } from "../../types";
...
function createSystem(): { agent: ValidationAgent; storage: Storage; memorySystem: MemorySystem } {
  const api = createApi();
...
describe("ValidationAgent.validate", () => {
  ...
  test("detects contradictions from memory system", async () => {
    ...
    storage.insertMemory({
      id: "m1", sessionId: "s1", bug: "bug1", investigation: "",
      rootCause: "cause_a", fix: "", reason: "",
      confidence: 0.9, tags: ["tag1"], files: [], commands: [],
    });
    storage.insertMemory({
      id: "m2", sessionId: "s1", bug: "bug2", investigation: "",
      rootCause: "cause_b", fix: "", reason: "",
      confidence: 0.9, tags: ["tag1"], files: [], commands: [],
    });
  ...
  test("detects circular references", async () => {
    ...
    storage.insertMemory({
      id: "m1", sessionId: "s1", bug: "b1", investigation: "",
      rootCause: "", fix: "", reason: "",
      confidence: 0.8, tags: [], files: [], commands: [],
    });
    storage.insertMemory({
      id: "m2", sessionId: "s1", bug: "b2", investigation: "",
      rootCause: "", fix: "", reason: "",
      confidence: 0.8, tags: [], files: [], commands: [],
    });
  ...
  test("detects hallucinated links (low confidence) via direct storage check", async () => {
    ...
    storage.insertMemory({
      id: "m1", sessionId: "s1", bug: "bug", investigation: "",
      rootCause: "", fix: "", reason: "",
      confidence: 0.8, tags: ["test"], files: [], commands: [],
    });
    storage.insertMemory({
      id: "m2", sessionId: "s1", bug: "bug2", investigation: "",
      rootCause: "", fix: "", reason: "",
  ...
  test("detects inference via direct access", async () => {
    ...
    storage.insertEvidence("ev1", "m1", "observed",
      "The bug is probably in the auth module because the error seems related", "log");
  ...
  test("overallHealth is warning with <= 3 issues", async () => {
    ...
    storage.insertMemory({
      id: "m1", sessionId: "s1", bug: "b1", investigation: "",
      rootCause: "cause_a", fix: "", reason: "",
      confidence: 0.9, tags: ["t"], files: [], commands: [],
    });
    storage.insertMemory({
      id: "m2", sessionId: "s1", bug: "b2", investigation: "",
      rootCause: "cause_b", fix: "", reason: "",
  ...
  test("overallHealth is critical with > 3 issues", async () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const m2 = `m${i}b`;
      storage.insertMemory({
        id: m1, sessionId: "s1", bug: `bug${i}a`, investigation: "",
        rootCause: `cause_${i}a`, fix: "", reason: "",
        confidence: 0.9, tags: ["t"], files: [], commands: [],
      });
      storage.insertMemory({
        id: m2, sessionId: "s1", bug: `bug${i}b`, investigation: "",
        rootCause: `cause_${i}b`, fix: "", reason: "",
        confidence: 0.9, tags: ["t"], files: [], commands: [],
      });
...
describe("ValidationAgent additional edge cases", () => {
  ...
  test("issues array contains descriptive messages for each problem type", async () => {
    ...
    storage.insertMemory({
      id: "m1", sessionId: "s1", bug: "b1", investigation: "",
      rootCause: "rc1", fix: "", reason: "",
      confidence: 0.9, tags: ["t"], files: [], commands: [],
    });
    storage.insertMemory({
      id: "m2", sessionId: "s1", bug: "b2", investigation: "",
      rootCause: "rc2", fix: "", reason: "",
      confidence: 0.9, tags: ["t"], files: [], commands: [],
    });
    storage.insertLink("l1", "m1", "m2", "same_cause", 0.8);
