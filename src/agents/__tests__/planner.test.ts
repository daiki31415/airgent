...
import type { AgentContext, ModelEntry, OpenCodeResponse, PipelineNode } from "../../types";
...
describe("PlannerAgent.selectNodes", () => {
  ...
  test("parses comma-separated response", async () => {
    const agent = new PlannerAgent(mockModel(), createApi("clarify, plan, generate, test, validate, report"));
    agent.init(sampleContext());
...
describe("PlannerAgent.analyzeTask", () => {
  ...
  test("handles task description with special characters", async () => {
    ...
    const nodes = await agent.analyzeTask("Fix #123: TypeError in <Component /> & process.env.API_KEY");
    expect(nodes).toContain("generate");
  });
});
