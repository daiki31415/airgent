...
import type { AgentContext, AgentMessage, ModelEntry, OpenCodeResponse, CompressedEntry } from "../../types";
...
function createApi(): OpenCodeAPI {
  return new (class extends OpenCodeAPI {
    chat = mock(async () => ({
      id: "api-resp", content: "mock", model: "m",
...
function createManager(): { agent: CompressionAgent; compressionManager: CompressionManager; storage: Storage } {
  const api = createApi();
...
describe("CompressionAgent additional edge cases", () => {
  ...
  test("groupMessages splits large messages into chunks", async () => {
    ...
    const messages = Array.from({ length: 20 }, (_, i) =>
      makeMessage("x".repeat(1000)) // ~250 tokens each, 20 * 250 = 5000 tokens total
    );
