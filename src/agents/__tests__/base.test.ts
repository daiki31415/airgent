...
import type { AgentRole, AgentContext, ModelEntry, OpenCodeResponse } from "../../types";
...
function mockApi(): OpenCodeAPI {
  const api = new (class extends OpenCodeAPI {
    chat = mock(async (_model: ModelEntry, _messages: Array<{ role: string; content: string }>): Promise<OpenCodeResponse> => {
      return {
        id: "test-id",
...
describe("BaseAgent.constructor", () => {
  ...
  test("accepts all valid AgentRole values", () => {
    const roles: AgentRole[] = [
      "worker", "planner", "memory_organizer", "compression",
      "validation", "watchdog", "context_inspector",
    ];
...
describe("BaseAgent.think", () => {
  ...
  test("works with empty user prompt", async () => {
    const api = mockApi();
    api.chat = mock(async () => ({
      id: "id", content: "response to empty", model: "m",
      usage: { promptTokens: 0, completionTokens: 0, totalTokens: 0 },
    }));
    const agent = new TestAgent("worker", mockModel(), api);
    agent.init(sampleContext());
  ...
  test("api.chat receives the model entry passed to constructor", async () => {
    const api = mockApi();
    const customModel: ModelEntry = { provider: "custom", model: "custom-model" };
...
describe("BaseAgent.thinkStream", () => {
  ...
  test("passes model to api.streamChat", async () => {
    ...
    for await (const _ of agent.callThinkStream("test")) { /* consume */ }
