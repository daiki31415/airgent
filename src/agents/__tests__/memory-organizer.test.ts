...
function createSystem(): { agent: MemoryOrganizerAgent; storage: Storage; memorySystem: MemorySystem } {
  const api = createApi();
...
describe("MemoryOrganizerAgent.organize", () => {
  ...
  test("creates memory from raw log with bug pattern", async () => {
    ...
    storage.insertRawLog("log1", "test-session", "worker",
      "Bug: login button not working on mobile", 10);
    storage.insertRawLog("log2", "test-session", "worker",
      "Fix: Added touch event handler", 10);
  ...
  test("creates memory with evidence entries", async () => {
    ...
    storage.insertRawLog("log1", "s1", "worker",
      "Bug: TypeError in parse function", 10);
    storage.insertRawLog("log2", "s1", "worker",
      "Fix: Added null check", 10);
  ...
  test("parses investigation lines from raw logs", async () => {
    ...
    storage.insertRawLog("log1", "s1", "worker",
      "Bug: Memory leak detected", 10);
    storage.insertRawLog("log2", "s1", "worker",
      "Investigation: Found circular reference in cache", 10);
    storage.insertRawLog("log3", "s1", "worker",
      "Reason: Cache entries never expired", 10);
    storage.insertRawLog("log4", "s1", "worker",
      "Fix: Added TTL to cache entries", 10);
  ...
  test("handles multiple bugs in consecutive logs", async () => {
    ...
    storage.insertRawLog("log1", "s1", "worker",
      "Bug: Error in auth", 10);
    storage.insertRawLog("log2", "s1", "worker",
      "Fix: Updated middleware", 10);
    storage.insertRawLog("log3", "s1", "worker",
      "Bug: Slow query", 10);
    storage.insertRawLog("log4", "s1", "worker",
      "Fix: Added index", 10);
  ...
  test("classifies evidence as observed by default", async () => {
    ...
    storage.insertRawLog("log1", "s1", "worker",
      "Bug: Crash on startup", 10);
    storage.insertRawLog("log2", "s1", "worker",
      "Fix: Updated config", 10);
  ...
  test("extracts tags from bug text", async () => {
    ...
    storage.insertRawLog("log1", "s1", "worker",
      "Bug: Performance issue with memory management", 10);
    storage.insertRawLog("log2", "s1", "worker",
      "Fix: Optimized cache", 10);
...
describe("MemoryOrganizerAgent with structured log patterns", () => {
  test("detects bug: pattern in logs", async () => {
    ...
    storage.insertRawLog("l1", "s1", "worker",
      "Bug: TypeError - cannot read property 'x' of undefined", 10);
    storage.insertRawLog("l2", "s1", "worker",
      "Fix: Added optional chaining", 10);
  ...
  test("handles root cause analysis logs", async () => {
    ...
    storage.insertRawLog("l1", "s1", "worker",
      "Bug: Data corruption", 10);
    storage.insertRawLog("l2", "s1", "worker",
      "root cause: Race condition in write path", 10);
    storage.insertRawLog("l3", "s1", "worker",
      "Fix: Added mutex lock", 10);
  ...
  test("handles logs with fix: pattern", async () => {
    ...
    storage.insertRawLog("l1", "s1", "worker",
      "Bug: UI not rendering", 10);
    storage.insertRawLog("l2", "s1", "worker",
      "fixed: Updated CSS selector", 10);
  ...
  test("handles logs with reason: pattern", async () => {
    ...
    storage.insertRawLog("l1", "s1", "worker",
      "Bug: Build failure", 10);
    storage.insertRawLog("l2", "s1", "worker",
      "because: Missing dependency in package.json", 10);
    storage.insertRawLog("l3", "s1", "worker",
      "Fix: Added dependency", 10);
  ...
  test("creates fallback pattern when no bug/fix detected", async () => {
    ...
    storage.insertRawLog("l1", "s1", "worker",
      "Just some random conversation about code", 10);
...
describe("MemoryOrganizerAgent calculateConfidence", () => {
  ...
  test("verified evidence yields high confidence", () => {
    const { agent } = createSystem();
    const calc = (agent as any).calculateConfidence.bind(agent);
    const result = calc([{ type: "verified", content: "test", source: "test", timestamp: 0 }]);
  ...
  test("observed evidence yields medium confidence", () => {
    const { agent } = createSystem();
    const calc = (agent as any).calculateConfidence.bind(agent);
    const result = calc([{ type: "observed", content: "log", source: "log", timestamp: 0 }]);
  ...
  test("capped at 1.0", () => {
    ...
    const manyVerified = Array.from({ length: 10 }, () => ({
      type: "verified" as const, content: "x", source: "x", timestamp: 0,
    }));
...
describe("MemoryOrganizerAgent extractTags", () => {
  test("extracts file extension tags", () => {
    ...
    const tags = extract({ bug: "bug in code", fix: "fix", files: ["src/main.ts", "src/style.css"] });
  ...
  test("extracts keyword tags from bug text", () => {
    const { agent } = createSystem();
    const extract = (agent as any).extractTags.bind(agent);
    const tags = extract({ bug: "Got a crash with memory error", fix: "", files: [] });
  ...
  test("deduplicates tags", () => {
    ...
    const extract = (agent as any).extractTags.bind(agent);
    const tags = extract({
      bug: "error crash error crash", fix: "",
...
describe("MemoryOrganizerAgent error handling", () => {
  test("handles session with only non-bug logs", async () => {
    ...
    storage.insertRawLog("l1", "s1", "worker",
      "Planning the implementation of feature X", 10);
    storage.insertRawLog("l2", "s1", "worker",
      "Generated code for feature X", 10);
