...
import type { AgentContext, ModelEntry, OpenCodeResponse, InspectionResult } from "../../types";
...
function createApi(): OpenCodeAPI {
  return new (class extends OpenCodeAPI {
    chat = mock(async () => ({
      id: "resp", content: "remediation suggestion", model: "m",
...
describe("ContextInspectorAgent.inspect", () => {
  ...
  test("detects same error repeated across inspections", () => {
    ...
    for (let i = 0; i < 3; i++) {
      agent.inspect({
        currentFocus: "Fix bug", errors: ["TypeError in parse"],
        todos: [], messages: [{ role: "user", content: "Fix it" }],
      });
    }
  ...
  test("detects purpose drift when focus diverges from original task", () => {
    ...
    const result = agent.inspect({
      ...
      todos: [],
      messages: [
        { role: "user", content: "Build a user authentication system with login, registration, and password reset" },
  ...
  test("detects stuck TODOs", () => {
    ...
    for (let i = 0; i < 3; i++) {
      agent.inspect({
        currentFocus: "Work", errors: [],
        todos: ["Fix login bug"], messages: [{ role: "user", content: "Help" }],
      });
    }
  ...
  test("no stuck TODO when TODOs change", () => {
    ...
    agent.inspect({
      currentFocus: "Work", errors: [],
      todos: ["Fix auth"], messages: [{ role: "user", content: "Help" }],
    });
    agent.inspect({
      currentFocus: "Work", errors: [],
      todos: ["Fix auth"], messages: [{ role: "user", content: "Help" }],
    ...
    const result = agent.inspect({
      currentFocus: "Work", errors: [],
      todos: ["Fix UI"], messages: [{ role: "user", content: "Help" }],
    });
  ...
  test("detects assumption fixation", () => {
    ...
    for (let i = 0; i < 3; i++) {
      agent.inspect({
        currentFocus: "Work", errors: [], todos: [],
  ...
  test("detects error change unrecognized when errors shift", () => {
    ...
    agent.inspect({
      currentFocus: "Fix", errors: ["TypeError"],
      todos: [], messages: [{ role: "user", content: "Fix" }],
    });
    agent.inspect({
      currentFocus: "Fix", errors: ["TypeError"],
      todos: [], messages: [{ role: "user", content: "Fix" }],
    ...
    const result = agent.inspect({
      currentFocus: "Fix", errors: ["SyntaxError", "ReferenceError"],
      todos: [], messages: [{ role: "user", content: "Fix" }],
    });
  ...
  test("no error change flag with single error change", () => {
    ...
    const result = agent.inspect({
      currentFocus: "Fix", errors: ["SyntaxError"],
      todos: [], messages: [{ role: "user", content: "Fix" }],
    });
  ...
  test("score is capped at 1.0", () => {
    ...
    agent.inspect({
      currentFocus: "Fix", errors: ["Err1", "Err2", "Err3"],
      todos: ["TODO1"], assumptions: ["Assumption1"],
      messages: [
        { role: "user", content: "Build a user authentication system with login registration and password reset" },
...
describe("ContextInspectorAgent edge cases", () => {
  test("handles empty errors array", () => {
    ...
    const result = agent.inspect({
      currentFocus: "Work", errors: [],
      todos: [], messages: [{ role: "user", content: "Help" }],
    });
    expect(result.score).toBe(0);
  });
  ...
  test("maintains max 20 previous states", () => {
    ...
    for (let i = 0; i < 25; i++) {
      agent.inspect({
        currentFocus: `Focus ${i}`, errors: [],
        todos: [], messages: [{ role: "user", content: "Test" }],
      });
    }
  ...
  test("handles inspection after many inspections", () => {
    ...
    for (let i = 0; i < 20; i++) {
      agent.inspect({
        currentFocus: `Focus ${i}`, errors: [],
        todos: [], messages: [{ role: "user", content: `Msg ${i}` }],
      });
    }

    const result = agent.inspect({
      currentFocus: "Final focus", errors: ["Error"],
      todos: ["TODO"], messages: [{ role: "user", content: "Final" }],
    });
...
describe("ContextInspectorAgent additional edge cases", () => {
  test("no error change when going from no errors to one error", () => {
    ...
    const result = agent.inspect({
      currentFocus: "Work", errors: ["NewError"],
      todos: [], messages: [{ role: "user", content: "Help" }],
    });
  ...
  test("error change flagged when multiple errors added and removed", () => {
    ...
    agent.inspect({
      currentFocus: "Work", errors: ["ErrA", "ErrB"],
      todos: [], messages: [{ role: "user", content: "Help" }],
    });
    agent.inspect({
      currentFocus: "Work", errors: ["ErrA", "ErrB"],
    ...
    const result = agent.inspect({
      currentFocus: "Work", errors: ["ErrC", "ErrD", "ErrE"],
      todos: [], messages: [{ role: "user", content: "Help" }],
    });
  ...
  test("score is cumulative when multiple flags are true", () => {
    ...
    const setupCalls = [
      { currentFocus: "Fix the auth bug in login", errors: ["TypeError"], todos: ["Fix login bug"], assumptions: [] as string[] },
      { currentFocus: "Fix the auth bug in login", errors: ["TypeError"], todos: ["Fix login bug"], assumptions: [] as string[] },
      { currentFocus: "Fix the auth bug in login", errors: ["TypeError"], todos: ["Fix login bug"], assumptions: [] as string[] },
    ];
    ...
    const result = agent.inspect({
      currentFocus: "Watching cat videos on youtube", errors: ["TypeError"],
      todos: ["Fix login bug"], assumptions: [],
      messages: [
        { role: "user", content: "Fix the authentication system login bug" },
        { role: "user", content: "m2" },
        { role: "user", content: "m3" },
...
describe("ContextInspectorAgent.suggestRemediation", () => {
  test("returns no remediation for low score", async () => {
    ...
    const result = await agent.suggestRemediation({
      sameErrorRepeated: false, purposeForgotten: false,
      todoStuck: false, assumptionFixed: false,
  ...
  test("calls think() when remediation is needed", async () => {
    ...
    const result = await agent.suggestRemediation({
      sameErrorRepeated: true, purposeForgotten: false,
      todoStuck: true, assumptionFixed: false,
      errorChangeUnrecognized: false,
  ...
  test("includes relevant issues in remediation prompt", async () => {
    ...
    await agent.suggestRemediation({
      sameErrorRepeated: true, purposeForgotten: true,
      todoStuck: false, assumptionFixed: false,
      errorChangeUnrecognized: false,
