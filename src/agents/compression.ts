...
export class CompressionAgent extends BaseAgent {
  ...
  async compress(messages: AgentMessage[], threshold = 0.7): Promise<{
    compressed: boolean; entries: CompressedEntry[]; originalCount: number; compressedCount: number; reduction: string;
  }> {
    ...
    if (usageRatio < threshold) {
      return { compressed: false, entries: [], originalCount: messages.length, compressedCount: 0, reduction: "0%" };
    }
    ...
    return {
      compressed: true, entries,
      originalCount: messages.length, compressedCount: entries.length,
