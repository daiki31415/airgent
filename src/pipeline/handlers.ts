...
export function registerPipelineHandlers(agent: PipelineHandle): void {
  agent.pipeline.registerHandler("clarify", async () => {
    const messages = [
      { role: "system" as const, content: agent.promptManager.buildNodePrompt("clarify") },
  ...
  agent.pipeline.registerHandler("plan", async () => {
    ...
    const messages = [
      { role: "system" as const, content: agent.promptManager.buildNodePrompt("plan") },
  ...
  agent.pipeline.registerHandler("generate", async () => {
    ...
    const result = await agent.worker.execute(generationPrompt,
  ...
  agent.pipeline.registerHandler("test", async () => {
    ...
    const messages = [
      { role: "system" as const, content: agent.promptManager.buildNodePrompt("test") },
      { role: "user" as const, content: `Task: ${agent.currentTask}\n\nOutput:\n${agent.pipelineData.generatedOutput.slice(0, 4000)}` },
    ];
    const content = await callLLM({
  ...
  agent.pipeline.registerHandler("validate", async () => {
    ...
    if (report.overallHealth !== "healthy") {
      agent.ui.log("warn", "validation", `Health: ${report.overallHealth} (${report.issues.length} issues)`);
    }
    return report;
  });
