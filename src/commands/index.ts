...
import type { MCPServerConfig, Settings, ModelEntry, ModelConfig } from "../types";
...
function handleSettingViewConfig(agent: AgentHandle): void {
  ...
  agent.ui.log("info", "config", `maxTokens: ${s.maxSystemPromptTokens} | context: ${s.maxContextTokens}`);
  agent.ui.log("info", "config", `debug: ${s.debug} | autoLink: ${s.memoryAutoLink} | progress: ${s.showPipelineProgress}`);
  agent.ui.log("info", "config", `retries: ${s.maxRetriesPerNode} | compress: ${s.autoCompressThreshold} | ui: ${s.uiRefreshIntervalMs}ms`);
}
...
export async function handleModelCommand(agent: AgentHandle, args: string[]): Promise<void> {
  if (args[0] === "all") {
    ...
    agent.ui.log("info", "airgent", "Usage: /model <role> | all — roles: " + ROLE_CONFIGS.map(r => r.key).join(", "));
  }
}

export async function handleMCPCommand(agent: AgentHandle, args: string[], line: string): Promise<void> {
  const sub = args[0];
  ...
  if (!sub || sub === "list") {
    try {
      ...
      agent.ui.log("info", "mcp", "Usage: /mcp add <name> local <command...> | add-remote <name> <url> | connect <name> | disconnect <name> | remove <name>");
    } catch (err) { agent.ui.log("error", "mcp", sanitizeError(err)); }
    return;
  }
  ...
  if (sub === "add") {
    ...
    try {
      await agent.api.addMCP(name, { type: "local", command: restArgs, enabled: true } as unknown as Record<string, unknown>);
  ...
  if (sub === "add-remote") {
    ...
    agent.configManager.saveMCPServers(servers);
    try {
      await agent.api.addMCP(name, { type: "remote", url, enabled: true } as unknown as Record<string, unknown>);
  ...
  if (sub === "connect") {
    const name = args[1];
    if (!name) { agent.ui.log("warn", "mcp", "Usage: /mcp connect <name>"); return; }
    try {
  ...
  if (sub === "disconnect") {
    const name = args[1];
    if (!name) { agent.ui.log("warn", "mcp", "Usage: /mcp disconnect <name>"); return; }
    try {
  ...
  if (sub === "remove") {
    const name = args[1];
    if (!name) { agent.ui.log("warn", "mcp", "Usage: /mcp remove <name>"); return; }
  ...
  if (lower.startsWith("/")) {
    ...
    switch (cmd) {
      case "/quit": case "/exit":
        ...
        return;
      case "/help":
        agent.ui.log("info", "airgent", "/quit /model /mcp /setting /status /session /compress /providers /sync /cat /copy /help /info");
        return;
      case "/info": {
        const health = await agent.api.healthCheck();
        agent.ui.notice("Commands: /quit /help /status /info /session /compress /providers /sync /cat /copy");
      ...
      case "/providers":
        try {
          ...
          agent.ui.log("info", "providers", "Available: " + providers.all.map(p => p.id).join(", "));
        } catch (err) {
      ...
      case "/sync":
        if (args[0] === "push") {
          ...
          try { agent.deviceSync.push(); agent.ui.log("info", "sync", "Push done"); }
          catch (err) { agent.ui.log("error", "sync", sanitizeError(err)); }
        } else if (args[0] === "pull") {
          try { agent.deviceSync.pull(); agent.ui.log("info", "sync", "Pull done"); }
          catch (err) { agent.ui.log("error", "sync", sanitizeError(err)); }
        } else {
      ...
      case "/cat":
        if (args[0]) {
          ...
          } catch (err) { agent.ui.log("error", "cat", sanitizeError(err)); }
        } else {
          agent.ui.log("info", "cat", "Usage: /cat <file>");
        }
        return;
      case "/copy": {
        const copyText = args.join(" ") || agent.pipelineData.generatedOutput || agent.pipelineData.plan || "";
        if (!copyText) {
...
async function fetchModelEntries(agent: AgentHandle): Promise<Array<{ name: string; description: string; value: ModelEntry }> | null> {
  let providers;
  try {
  ...
  if (entries.length === 0) {
    ...
    agent.ui.log("warn", "airgent", "Run `opencode serve` then /connect in the OpenCode TUI to add models.");
    return null;
  }
